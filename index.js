...
class Browser {
  constructor(options = {}) {
    ...
    else if (!("nodeIntegration" in options.webPreferences))
      options.webPreferences.nodeIntegration = false;

    let script = "";
    if (typeof options.exposeElectronAs === "string")
      script = `window[${JSON.stringify(
        options.exposeElectronAs
      )}] = require('electron');\n`;
    if (typeof options.preloadScript === "string")
      script += options.preloadScript;
    if (script) {
